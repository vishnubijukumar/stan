...
Step 5 seems a lot, but it's alright. After we edit the step 5 from the tutorial to our stan.c, we dig deep into `man tcgetattr` and find out what `TCSAFLUSH` and `ECHO` (ECHO is one of the constraint under `c_lflag` flag) is:
...
Now, after quitting stan (it responds to 'q' in the input btw) our bash shell got stuck since it was acting as if it was still running stan. The tutorial says what to do if this happens -
...
This step seemed as if stan is imbued with magic. See for yourself:
...
Now we can only exit stan by pressing 'q' and not `Ctrl+C` or `Ctrl+Z`.
...
We see that we our stan's `c_lflag` matches the flags from the above doc.
...
This is something which didn't happen in my terminal when stan was running. Nevertheless, I will be adding the flag `IEXTEN` to the c_lflag.
...
As we can see after this step, our stan prints out `0` in a continuously manner. Step 16 fully turns our terminal into raw mode.

-----

## step 17 - adding the `die()` function

This `die()` function prints the error message and ends stan.
...
An easy way to make `tcgetattr()` fail is to give your program a text file or a pipe as the standard input instead of your terminal. To give it a file as standard input, run `./stan <stan.c`. To give it a pipe, run `echo test | ./stan`. Both should result in the same error from `tcgetattr()`, something like `Inappropriate ioctl for device`.
...
## Step 19 - divided stan.c into sections

----

## Step 20 - making `Ctrl + q` stop stan instead of `q`
...
So, when stan was running in step 19, it printed out `Ctrl+q` as 17. And right now to exit our code, the terminal should read `17` in order to quit.
